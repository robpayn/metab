# this script is used to complete a Monte Carlo Sensitivity Analysis of a coupled DO and pCO2 river metab model
...
# Set known standard error for DO and pCO2
knownSdErrorDO <- 0.05;
knownSdErrorpCO2 <- 8.75;

#create function that allows us to easily change parameters in oneStationMetabMulti model
#
# argument
...
# write(x = c("dailyGPP", "dailyER", "k600"), file = "monteCarloEnsembleNlmrDataGPPof2LowN.csv",
...
# use nlm to minimize -logLikelyhood using obsSynth and the modelFunc
  ...
  # fill data frame with estimated parameters
...
# fill columns of previously created csv file with parameter estimations
  write(x = nlmr$estimate, file = "monteCarloEnsembleNlmrDataGPPof2LowN.csv", 
        ncolumns = 3, append = TRUE, sep = ",")
  
  }

# plot example of one iterations of coupled monte carlo
